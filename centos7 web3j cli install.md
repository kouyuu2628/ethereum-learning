...
```
老规矩，网站连接失败，只能自己从github下载压缩包手动安装
```
...
```
## 执行安装脚本
将下载的`web3j-4.5.4.tar`和脚本`web3j-cli-install.sh`放到`/root/.web3j/`，执行安装脚本
```
sudo bash web3j-cli-install.sh
```
指令`curl -L https://get.web3j.io | sh`里面有安装脚本，但是由于访问权限问题导致失败。  
`web3j-cli-install.sh`脚本是从官网改编而来，移除了下载部分。如果下载的版本号有变更，脚本里的版本号也要对应修改。
## 环境变量
```
